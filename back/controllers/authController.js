...
export const register = async (req, res) => {
    try {
        ...
        res.status(201).send({ error: false, body: { id: result[0].insertId }, message: 'Usuario creado exitosamente' });
    } catch (error) {
        console.error(error);
...
export const login = async (req, res) => {
    try {
        ...
        if (!rows.length) {
            throw ("El correo o la constraseña son incorrectas.")
        }
        ...
        res.status(200).send({error: false, body: {token}, message: "Inicio de sesión exitoso."})
    } catch (error) {
        console.error(error);
        res.status(error.status || 500).send({ error: true, body: null, message: error.message || error });
    }
